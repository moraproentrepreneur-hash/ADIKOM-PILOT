...
/**
 * Ouverture d'une photo d'état des lieux.
 *
 * Même chemin de passage que les documents de véhicule (migration 019), même
 * bucket privé sans policy : le navigateur ne peut pas lire un objet, même en
 * connaissant son chemin. La photo n'est accessible que par ici, qui :
 *
 *   1. exige une session — le proxy applicatif l'impose déjà ;
 *   2. vérifie `rental.rentals.view` : une photo de départ appartient au
 *      contrat, et se consulte avec lui ;
...
export async function GET(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!user.isSuperAdmin && !(await can(PERMISSIONS.RENTALS_VIEW))) {
  ...
  if (error) {
    console.error(`[états des lieux] lecture : ${error.code ?? 'ERREUR'} ${error.message}`)
  ...
  if (signError || !signed) {
    console.error(`[états des lieux] signature : ${signError?.message ?? 'inconnue'}`)
    return NextResponse.json({ error: 'La photo n’a pas pu être ouverte.' }, { status: 500 })
  }
